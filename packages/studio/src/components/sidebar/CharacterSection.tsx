...
const TIER_BADGE: Record<RoleRef["tier"], { zh: string; ko: string; en: string; color: string }> = {
  ...
  minor: { zh: "次要", ko: "조연", en: "Minor", color: "bg-blue-500/15 text-blue-600 dark:text-blue-400" },
};
    ...
    <div className="rounded-lg bg-secondary/30 overflow-hidden">
      ...
      {expanded && (
        <div className="px-2.5 pb-2.5 space-y-1">
          {tags && (
            <p className="text-[14px] leading-6 text-muted-foreground"><span className="text-muted-foreground/60">{tr("标签", "태그", "Tags")}</span> {tags}</p>
          )}
          {current && (
            <p className="text-[14px] leading-6 text-muted-foreground"><span className="text-muted-foreground/60">{tr("当前", "현재", "Current")}</span> {current}</p>
          )}
