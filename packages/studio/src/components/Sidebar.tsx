  ...
  return (
    <aside className="w-[260px] shrink-0 border-r border-border bg-background/80 backdrop-blur-md flex flex-col h-full overflow-hidden select-none">
              ...
              return (
                <div key={book.id}>
                  ...
                  <div className="group/book flex items-center">
                    <button
                      type="button"
                      aria-label={isExpanded ? tr(`折叠 ${book.title}`, `${book.title} 접기`, `Collapse ${book.title}`) : tr(`展开 ${book.title}`, `${book.title} 펼치기`, `Expand ${book.title}`)}
                  ...
                  <Collapse open={isExpanded}>
                    <div className="mt-0.5">
                      ...
                      >
                        <Plus size={12} />
                        <span>{tr("新建会话", "새 세션", "New session")}</span>
                      </button>
                    </div>
                  </Collapse>
                </div>
        ...
        <div data-testid="film-projects-section">
          ...
          <Collapse open={filmsExpanded}>
            <div className="space-y-0.5 pt-1">
              ...
              {films.length === 0 && (
                <div className="px-3 py-6 text-xs text-muted-foreground/50 italic text-center">
                  {tr("还没有互动影游项目", "아직 인터랙티브 영화 프로젝트가 없습니다", "No interactive film projects yet")}
                </div>
              )}
            </div>
          </Collapse>
      ...
      >
        ...
        >
          <DialogHeader className="space-y-0 gap-0">
            <DialogTitle className="font-sans text-sm font-medium">{tr("重命名会话", "세션 이름 변경", "Rename Session")}</DialogTitle>
          </DialogHeader>
          <input
            ...
            placeholder={tr("输入新标题", "새 제목 입력", "Enter a new title")}
      ...
      <ConfirmDialog
        ...
        title={tr("删除会话", "세션 삭제", "Delete Session")}
        message={tr(
          `确认删除“${deleteTarget?.title ?? ""}”吗？该操作只删除这条会话，不影响书籍内容。`,
          `"${deleteTarget?.title ?? ""}"을(를) 삭제하시겠습니까? 이 작업은 세션만 삭제하며 책 내용에는 영향을 주지 않습니다.`,
          `Delete "${deleteTarget?.title ?? ""}"? This only removes the session; the book content is not affected.`,
        )}
        confirmLabel={tr("删除", "삭제", "Delete")}
        cancelLabel={tr("取消", "취소", "Cancel")}
        variant="danger"
...
function getSessionLabel(session: { sessionId: string; title: string | null; messages: ReadonlyArray<{ role: string; content: string }> }): string {
  ...
  return tr("新会话", "새 세션", "New session");
}
...
function formatRelativeTime(sessionId: string): string {
  ...
  if (minutes < 1) return tr("刚刚", "방금 전", "just now");
  if (minutes < 60) return tr(`${minutes} 分钟`, `${minutes}분`, `${minutes}m`);
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return tr(`${hours} 小时`, `${hours}시간`, `${hours}h`);
  const days = Math.floor(hours / 24);
  if (days < 30) return tr(`${days} 天`, `${days}일`, `${days}d`);
  const months = Math.floor(days / 30);
  return tr(`${months} 个月`, `${months}개월`, `${months}mo`);
}
