    ...
    case "processing":
      return (
        <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span>{tr("处理结果", "결과 처리 중", "Processing result")}</span>
        </span>
      );
    case "completed":
      return (
        <span className="inline-flex items-center gap-1 text-xs text-green-600 dark:text-green-400">
          ...
          <span>{tr("已完成", "완료됨", "Completed")}</span>
        </span>
      );
    case "error":
      return (
...
function formatProgress(progress: NonNullable<PipelineStage["progress"]>): string {
  ...
  const statusLabel = progress.status === "thinking" ? tr("思考中", "생각 중", "Thinking") : progress.status ?? "";
...
function ScriptStoryboardResultPreview({ exec, onOpenFilmStudio }: { exec: ToolExecution; onOpenFilmStudio?: (projectId: string) => void }) {
  ...
  const maybeRows: Array<readonly [string, string] | null> = [
    details.specPath ? [tr("规格", "규격", "Spec"), details.specPath] : null,
    details.storyGraphPath ? [tr("剧情图谱", "스토리 그래프", "Story graph"), details.storyGraphPath] : null,
    details.storyTreePath ? [tr("剧情树", "스토리 트리", "Story tree"), details.storyTreePath] : null,
    details.flagsPath ? [tr("变量旗标", "변수 플래그", "Flags"), details.flagsPath] : null,
    details.scriptPath ? [tr("剧本", "대본", "Script"), details.scriptPath] : null,
    details.storyboardPath ? [tr("分镜", "스토리보드", "Storyboard"), details.storyboardPath] : null,
    details.imagePromptsPath ? [tr("图像提示词", "이미지 프롬프트", "Image prompts"), details.imagePromptsPath] : null,
    details.assetsManifestPath ? [tr("图片资产", "이미지 에셋", "Image assets"), details.assetsManifestPath] : null,
  ];
    ...
    <div className="mx-3 mb-3 mt-1 rounded-xl border border-primary/20 bg-primary/5 px-3 py-2.5">
      <div className="flex items-center justify-between gap-3">
        <div className="text-[16px] leading-6 font-semibold text-primary">
          {details.kind === "script_created"
            ? tr("剧本已生成", "대본 생성됨", "Script generated")
            : details.kind === "storyboard_created"
              ? tr("分镜已生成", "스토리보드 생성됨", "Storyboard generated")
              : tr("互动影游已生成", "인터랙티브 영화 생성됨", "Interactive film generated")}
        </div>
        {details.kind === "interactive_film_created" && details.projectId && onOpenFilmStudio && (
          ...
          >
            {tr("打开创作向导 →", "창작 위자드 열기 →", "Open creation wizard →")}
          </button>
        )}
      </div>
      {rows.length > 0 && (
...
function ShortFictionResultPreview({ exec }: { exec: ToolExecution }) {
  ...
  if (!coverPath || !/\.(png|jpe?g|webp)$/iu.test(coverPath)) {
    ...
    return (
      <div className="mx-3 mb-3 mt-1 rounded-xl border border-destructive/20 bg-destructive/5 px-3 py-2 text-xs text-destructive">
        {tr("封面未生成：", "커버 미생성: ", "Cover not generated: ")}{coverError}
      </div>
    );
  }
  ...
  const title = details?.title ?? details?.storyId ?? tr("短篇封面", "단편 표지", "Short fiction cover");
...
function PlaySceneImagePreview({ details }: { details: PlayToolDetails }) {
  ...
  return (
    <div className="mt-3 overflow-hidden rounded-xl border border-border/40 bg-background/80">
      <img
        src={readyUrl}
        alt={tr("本幕配图", "본 막 일러스트", "Scene illustration")}
      ...
      {details.turn != null && (
        <div className="border-t border-border/40 px-3 py-2.5 text-[14px] leading-6 text-muted-foreground">
          {tr(`第 ${Math.trunc(details.turn)} 幕配图`, `제 ${Math.trunc(details.turn)} 막 일러스트`, `Scene ${Math.trunc(details.turn)} illustration`)}
        </div>
      )}
    </div>
  );
...
export function getProposedActionContractRows(details: ProposedActionDetails): ReadonlyArray<{ label: string; value: string }> {
  ...
  if (worldContract) rows.push({ label: tr("世界契约", "세계 계약", "World contract"), value: worldContract });
  const visualContract = playStart.visualContract?.trim();
  if (visualContract) rows.push({ label: tr("视觉契约", "비주얼 계약", "Visual contract"), value: visualContract });
  return rows;
}
...
function ProposedActionPreview({
  ...
  return (
    <div className="mx-3 mb-3 mt-1 rounded-xl border border-primary/25 bg-primary/5 px-4 py-3.5">
      <div className="text-[17px] leading-6 font-semibold text-foreground">{details.title ?? tr("确认执行", "실행 확인", "Confirm action")}</div>
      {details.summary && (
      ...
      {resolution === "confirmed" ? (
        <div className="mt-3 flex items-center gap-1.5 text-[15px] leading-6 font-medium text-primary">
          ...
          {details.targetRoute ? tr("已打开", "열림", "Opened") : tr("已执行", "실행됨", "Executed")}
        </div>
      ) : resolution === "rejected" ? (
        <div className="mt-3 text-[15px] leading-6 font-medium text-muted-foreground">{tr("已取消", "취소됨", "Cancelled")}</div>
      ) : (
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          >
            {streaming ? tr("执行中…", "실행 중…", "Running…") : details.targetRoute ? tr("打开入口", "입구 열기", "Open entry") : tr("继续执行", "계속 실행", "Continue")}
          </button>
          <button
            type="button"
...
function PlayResultPreview({ exec }: { exec: ToolExecution }) {
  ...
  const label = details.kind === "play_world_started"
    ? tr("互动世界已启动", "인터랙티브 월드 시작됨", "Interactive world started")
    : details.kind === "play_turn_revised"
      ? tr("互动回合已重做", "플레이 턴 재실행됨", "Play turn redone")
      : details.kind === "play_variant_restored"
        ? tr("已切换互动回合版本", "인터랙티브 턴 버전 전환됨", "Switched play turn variant")
        : tr("互动世界已推进", "인터랙티브 월드 진행됨", "Interactive world advanced");
  return (
...
function PlayEditPreview({ exec }: { exec: ToolExecution }) {
  ...
  if (!details) return null;
  const changes = [
    details.updatedWorldContract ? tr("世界契约", "세계 계약", "World contract") : "",
    details.updatedVisualContract ? tr("视觉契约", "비주얼 계약", "Visual contract") : "",
    details.updatedPremise ? tr("世界前提", "세계 전제", "World premise") : "",
    details.updatedEntities && details.updatedEntities > 0
      ? tr(`${details.updatedEntities} 张卡片`, `${details.updatedEntities}개 카드`, `${details.updatedEntities} cards`)
      : "",
  ].filter(Boolean);
  return (
    <div className="mx-3 mb-3 mt-1 rounded-xl border border-primary/20 bg-primary/5 px-3 py-2.5">
      <div className="text-[16px] leading-6 font-semibold text-primary">{tr("互动世界设定已更新", "인터랙티브 월드 설정 업데이트됨", "Interactive world settings updated")}</div>
      <div className="mt-1 text-xs leading-5 text-muted-foreground">
        {changes.length > 0 ? changes.join(" · ") : tr("已写入当前世界。", "현재 월드에 기록됨.", "Written to the current world.")}
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <summary className="cursor-pointer select-none font-medium text-muted-foreground hover:text-foreground">
        {tr("查看操作结果", "작업 결과 보기", "View result")}
      </summary>
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-muted/50 transition-colors cursor-pointer text-xs text-muted-foreground">
        <Wrench size={12} />
        <span>{tr(`${execs.length} 个文件操作`, `${execs.length}개 파일 작업`, `${execs.length} file operation${execs.length === 1 ? "" : "s"}`)}</span>
