...
export default function FlowView({
  ...
  const onConnect = async (conn: { source: string | null; target: string | null }) => {
    ...
    if (!src) return;
    await post(addChoiceDelta(src, { id: genChoiceId(), text: tr("新选项", "새 선택지", "New choice"), targetNodeId: conn.target }));
  };
  ...
  return (
    <div className="flex flex-col h-full p-5 gap-3" data-testid="flow-view">
      <div className="flex items-center gap-3 text-sm shrink-0">
        ...
        >
          {editing ? tr("完成编辑", "편집 완료", "Done editing") : tr("编辑", "편집", "Edit")}
        </button>
        {editing && (
          <button
      ...
      {stats && (
        ...
        >
          <span>{tr("总节点", "전체 노드", "Nodes")} {stats.total}</span>
          <span>{tr("分支", "분기", "Branches")} {stats.branch}</span>
          <span>{tr("结局", "결말", "Endings")} {stats.ending}</span>
          <span>{tr("死路", "막다른 길", "Dead ends")} {stats.deadEnd}</span>
