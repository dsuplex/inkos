...
const REQUIRED_SECTIONS: ReadonlyArray<RequiredSection> = [
  ...
  { zh: "## 读者此刻在等什么", ko: "## 독자가 지금 기대하는 것", en: "## What the reader is waiting for right now", minContentChars: 20 },
  { zh: "## 该兑现的 / 暂不掀的", ko: "## 이번 장에서 둘어낼 것 / 미루어 둘 것", en: "## To pay off / to keep buried", minContentChars: 20 },
  { zh: "## 日常/过渡承担什么任务", ko: "## 일상/전환 비트가 담당하는 역할", en: "## What the slow / transitional beats carry", minContentChars: 20 },
  { zh: "## 关键抉择过三连问", ko: "## 핵심 선택의 3가지 질문", en: "## Three-question check on the key choice", minContentChars: 20 },
  { zh: "## 章尾必须发生的改变", ko: "## 챕터 끝에 반드시 일어나야 할 변화", en: "## Required end-of-chapter change", minContentChars: 20 },
  { zh: "## 本章 hook 账", ko: "## 이번 장 훅 장부", en: "## Hook ledger for this chapter", minContentChars: 20 },
  { zh: "## 不要做", ko: "## 하지 말 것", en: "## Do not", minContentChars: 1 },
];

const GOAL_HEADINGS = ["## 本章目标", "## 챕터 목표", "## Chapter goal"] as const;
const THREAD_HEADINGS = ["## 关联线索", "## 관련 실마리", "## Thread refs", "## Related threads"] as const;
...
function prependFullGoalIfNeeded(markdown: string, body: string, fullGoal: string, displayGoal: string): string {
  ...
  const heading = markdown.includes("## 챕터 목표")
    ? "## 챕터 목표"
